 ...
 */
function parseMcpToolsListTtlMs(): number | null {
...
if (!hasOAuthPair && !bootstrapToken) {
  throw new Error(
    'Missing MCP OAuth credentials: set CLIENT_ID and CLIENT_SECRET, or set BOOTSTRAP_TOKEN for a one-time bootstrap (then add id/secret to .env).',
  );
}
