...
/** Shape we accept from the client for each history row (OpenAI-compatible roles + string content). */
...
chatRouter.post('/chat', async (req: Request, res: Response) => {
  const raw = req.body as { messages?: IncomingMsg[]; user_context?: unknown };
  ...
  let userContextSystem: string | null = null;
  ...
  try {
    ...
    const oaMessages: ChatCompletionMessageParam[] = [
      {
        role: 'system',
        content:
          'You are a helpful assistant with access to MCP tools exposed by the connected server. Use tools when they help answer the user. Be concise and accurate.',
      },
      ...(userContextSystem
        ? ([{ role: 'system', content: userContextSystem }] as ChatCompletionMessageParam[])
        : []),
    ...
    while (rounds < MAX_TOOL_ROUNDS) {
      ...
      for (const tc of toolCalls) {
        ...
        const toolText = stringifyToolResult(result as { content?: unknown; toolResult?: unknown });
